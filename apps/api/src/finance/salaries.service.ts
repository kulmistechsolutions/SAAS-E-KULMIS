...
export class SalariesService {
  ...
  /**
   * What deleting a whole payroll month would destroy, without touching it.
   * The UI shows this before asking to confirm, because rows that have money
   * against them take their SalaryPayment ledger with them (FK cascade).
   */
  async monthDeletionPreview(schoolId: string, year: number, month: number) {
    return this.prisma.forTenant(schoolId, async (tx) => {
      const rows = await tx.salary.findMany({
        where: { year, month },
        select: { id: true, amountPaid: true },
      });
      const paidRows = rows.filter((r) => r.amountPaid > 0);
      return {
        year,
        month,
        totalRows: rows.length,
        rowsWithPayments: paidRows.length,
        amountAlreadyPaid: paidRows.reduce((s, r) => s + r.amountPaid, 0),
      };
    });
  }

  /**
   * Delete an entire payroll month — the Danger Zone action, for a month that
   * was generated by mistake.
   *
   * Rows that already have money against them are refused unless the caller
   * explicitly confirms, since deleting them also deletes their payment
   * ledger and silently changes what the finance reports say was spent.
   */
  async removeMonth(
    schoolId: string,
    year: number,
    ...
    const preview = await this.monthDeletionPreview(schoolId, year, month);
    if (preview.totalRows === 0) {
      throw new NotFoundException(
        `No payroll exists for ${String(month).padStart(2, "0")}/${year}.`,
      );
    }
    if (preview.rowsWithPayments > 0 && !opts.includePaid) {
      throw new ConflictException(
        `${preview.rowsWithPayments} payroll row(s) for ${String(month).padStart(2, "0")}/${year} ` +
          `already have $${preview.amountAlreadyPaid} paid against them. Deleting them also deletes ` +
          "their payment history — confirm explicitly to proceed.",
      );
    }

    return this.prisma.forTenant(schoolId, async (tx) => {
      const where = opts.includePaid
        ? { year, month }
        : { year, month, amountPaid: 0 };
      const { count } = await tx.salary.deleteMany({ where });
      return {
        deleted: count,
        year,
        month,
        amountAlreadyPaidRemoved: opts.includePaid
          ? preview.amountAlreadyPaid
          : 0,
      };
    });
  }
