...
const QUIZ_CHILDREN: NavChild[] = [
  ...
  { label: "Question Bank", href: "/quiz/question-bank" },
  { label: "Student Portal", href: "/quiz/portal" },
...
const NAV: NavItem[] = [
  ...
  { label: "Parent Portal", icon: HeartHandshake, href: "/parent-portal/login" },
  {
    ...
    icon: Library,
    children: ACADEMICS_CHILDREN,
  },
  { label: "Attendance", icon: CalendarCheck, href: "/attendance" },
  {
    label: "Fee Management",
    icon: Wallet,
    children: FEE_CHILDREN,
  },
  {
    label: "Salary Management",
    icon: Receipt,
    children: SALARY_CHILDREN,
  },
  {
    ...
    <div className="flex h-full flex-col bg-[#111a2e] text-slate-300">
      ...
      >
        ...
        {!collapsed && (
          <div className="min-w-0 leading-tight">
            ...
            <p className="truncate text-[11px] text-slate-400">{branding.tagline}</p>
          </div>
        )}
        {onToggleCollapse && (
          <button
      ...
      <nav className="scrollbar-slim flex-1 space-y-1 overflow-y-auto overflow-x-hidden px-3 py-4">
        {NAV.map((item) => {
          ...
          const active =
            item.href &&
            (item.href === "/finance" || item.href === "/examinations" || item.href === "/academics" || item.href === "/promotions"
              ? pathname === item.href
