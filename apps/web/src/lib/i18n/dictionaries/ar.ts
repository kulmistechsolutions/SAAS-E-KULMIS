import type { PartialDictionary } from "./en";
import { arGenerated } from "./ar-generated";
import { mergeDictionaries } from "./merge";

/**
 * العربية. The interface flips to RTL for this language — see dirOf() in the
 * i18n config and the `dir` attribute set on <html>.
 */
const curated: PartialDictionary = {
  common: {
    defaultGrades: "الصفوف الافتراضية (Grade)",
    save: "حفظ",
    saving: "جارٍ الحفظ…",
    cancel: "إلغاء",
    close: "إغلاق",
    delete: "حذف",
    edit: "تعديل",
    view: "عرض",
    add: "إضافة",
    create: "إنشاء",
    update: "تحديث",
    search: "بحث",
    filter: "تصفية",
    clear: "مسح",
    export: "تصدير",
    import: "استيراد",
    print: "طباعة",
    download: "تنزيل",
    upload: "رفع",
    back: "رجوع",
    next: "التالي",
    previous: "السابق",
    submit: "إرسال",
    confirm: "تأكيد",
    yes: "نعم",
    no: "لا",
    all: "الكل",
    none: "لا شيء",
    loading: "جارٍ التحميل…",
    noData: "لا توجد بيانات بعد",
    noResults: "لا توجد نتائج",
    actions: "إجراءات",
    status: "الحالة",
    active: "نشط",
    inactive: "غير نشط",
    name: "الاسم",
    fullName: "الاسم الكامل",
    phone: "الهاتف",
    email: "البريد الإلكتروني",
    address: "العنوان",
    gender: "الجنس",
    male: "ذكر",
    female: "أنثى",
    date: "التاريخ",
    total: "الإجمالي",
    amount: "المبلغ",
    notes: "ملاحظات",
    required: "مطلوب",
    optional: "اختياري",
    selected: "تم تحديد {count}",
    retry: "حاول مرة أخرى",
    somethingWentWrong: "حدث خطأ ما",
  },

  platformAi: {
    aiGradingOpenai: "ميزات الذكاء الاصطناعي",
    platformWideOpenaiKeyUsedTo: "مفتاح واحد للمنصة بأكملها: تصحيح الاختبارات، وتقارير مساعد المدرسة المكتوبة، وقواعد الجدول.",
    provider: "المزود",
    providerHelp: "كلاهما يستخدم نفس الواجهة. يوفر OpenRouter نماذج متعددة بمفتاح واحد، بما فيها المجانية.",
    keyHintOpenai: "يبدو المفتاح مثل sk-…",
    keyHintOpenrouter: "يبدو المفتاح مثل sk-or-v1-… — احصل عليه من openrouter.ai/keys",
    modelHintOpenai: "مثال: gpt-4o-mini",
    modelHintOpenrouter: "تحمل نماذج OpenRouter اسم صانعها: openai/gpt-4o-mini أو google/gemini-2.0-flash-001 أو نموذج :free.",
    providerChanged: "تم تغيير المزود — احفظ ثم اختبر الاتصال.",
    saving: "جارٍ الحفظ…",
    saveSettings: "حفظ الإعدادات",
    notTested: "لم يُختبر",
  },
  copilot: {
    title: "مساعد المدرسة",
    subtitle: "كل ما تم تسجيله من {from} إلى {to}{year}، في مكان واحد.",
    subtitleFallback: "أرقام مدرستك، في مكان واحد.",
    loadFailed: "تعذر تحميل أرقام المدرسة.",
    refresh: "تحديث",
    loading: "جارٍ التحميل...",
    briefTitle: "تقرير هذا الشهر",
    briefFooter: "كُتب من الأرقام المسجلة لهذه المدرسة من {from} إلى {to}. لا يُرسل اسم طالب أو ولي أمر أو موظف.",
    briefUnavailable: "التقرير المكتوب غير متاح الآن - الأرقام أدناه غير متأثرة.",
    students: "الطلاب",
    maleFemale: "{male} ذكور · {female} إناث",
    collected: "المُحصّل هذا الشهر",
    collectedHint: "{today} اليوم · {rate} من المتوقع",
    attendance: "الحضور",
    attendanceHint: "{inCount} حضروا اليوم، {outCount} غائبون",
    netIncome: "صافي الدخل",
    netIncomeHint: "{inAmount} داخل · {outAmount} خارج",
    vsLastMonth: "{delta} نقطة مقارنة بالشهر الماضي",
    moneyTitle: "أموال هذا الشهر",
    feesCollected: "الرسوم المُحصّلة",
    additionalIncome: "دخل إضافي",
    salariesPaid: "الرواتب المدفوعة",
    expenses: "المصروفات",
    feeCollection: "تحصيل الرسوم",
    outstandingAll: "{amount} لا تزال مستحقة عبر جميع الأشهر.",
    payrollTitle: "الرواتب بالتفصيل",
    payrollDue: "إجمالي المستحق للموظفين",
    payrollPaid: "المدفوع حتى الآن",
    payrollLeft: "المتبقي",
    payrollStaff: "{paid} من {total} موظفًا دُفع لهم بالكامل",
    expenseBreakdown: "أين ذهبت الأموال",
    incomeBreakdown: "من أين جاءت الأموال الإضافية",
    entries: "{count} إدخالات",
    nothingRecorded: "لم يُسجّل شيء هذا الشهر.",
    sixMonths: "المفوتر مقابل المحصّل، آخر ستة أشهر",
    billed: "المفوتر",
    collectedCol: "المحصّل",
      paidFull: "دفعوا بالكامل",
    paidPart: "دفعوا جزءًا",
    paidNone: "لم يدفعوا شيئًا",
  askTitle: "اسأل عن مدرستك",
    askHelp: "تتم الإجابة فقط مما سجلته هذه المدرسة - دون اختلاق. إذا لم تحتوِ الأرقام على الإجابة، فسيوضح السجل الناقص.",
    askPlaceholder: "كيف يسير تحصيل الرسوم مقارنة بالشهر الماضي؟",
    askButton: "اسأل",
    asking: "جارٍ السؤال...",
    quotaLeft: "بقي {remaining} من {limit} أسئلة اليوم",
    limitReached: "استخدمت هذه المدرسة كل أسئلتها لليوم. الأرقام أعلاه تبقى متاحة، وتعود الحصة غدًا.",
    askUnavailable: "خدمة الكتابة لا تستجيب الآن. الأرقام أعلاه غير متأثرة.",
    askFailed: "تعذر إرسال السؤال. لا شيء آخر في هذه الصفحة يتأثر.",
    historyTitle: "الأسئلة السابقة ({count})",
    schoolToday: "المدرسة اليوم",
    teachers: "المعلمون",
    teacherAtt: "{rate} حضور هذا الشهر",
    parents: "أولياء الأمور",
    parentsHint: "لديهم حسابات في البوابة",
    classesSections: "الفصول / الشُعب",
    examsRecorded: "{count} امتحانات مسجلة",
    newStudents: "طلاب جدد",
    newStudentsHint: "تسجلوا هذا الشهر",
    quizzesSat: "الاختبارات المؤداة",
    quizNone: "لم يُصحح أي منها بعد",
    quizHint: "{avg} المتوسط · {pass} ناجحون",
    topStudents: "أفضل الطلاب",
    needsAttention: "بحاجة إلى اهتمام",
    lowAttendance: "الحضور أقل من 75%",
    owingMost: "الأكثر مديونية",
    nothingOutstanding: "لا شيء مستحق.",
    noneYet: "لم يُسجّل ما يكفي بعد.",
  },
  nav: {
    copilot: "مساعد المدرسة",
    dashboard: "لوحة التحكم",
    students: "الطلاب",
    teachers: "المعلمون",
    parents: "أولياء الأمور",
    parentPortal: "بوابة ولي الأمر",
    teacherPortal: "بوابة المعلم",
    classesSections: "الصفوف والشعب",
    attendance: "الحضور",
    studentCases: "حالات الطلاب",
    timetable: "الجدول الدراسي",
    feeManagement: "إدارة الرسوم",
    salaryManagement: "إدارة الرواتب",
    expenseManagement: "إدارة المصروفات",
    examinations: "الامتحانات",
    promotions: "الترفيع",
    onlineQuiz: "الاختبار الإلكتروني",
    finance: "المالية",
    sms: "الرسائل النصية",
    smsPackages: "باقات الرسائل",
    library: "المكتبة",
    idGenerator: "مولّد البطاقات",
    reports: "التقارير",
    usersRoles: "المستخدمون والصلاحيات",
    settings: "الإعدادات",
    systemLogs: "سجلات النظام",

    myProfile: "ملفي الشخصي",
    myStudents: "طلابي",
    myTimetable: "جدولي",
    myAssignments: "مهامي",
    announcements: "الإعلانات",

    classes: "الصفوف",
    sections: "الشعب",
    subjects: "المواد",
    academicYears: "الأعوام الدراسية",
    timetables: "الجداول",
    setup: "الإعداد",
    collectFees: "تحصيل الرسوم",
    feeHistory: "سجل الرسوم",
    freeStudents: "الطلاب المجانيون",
    monthlySetup: "الإعداد الشهري",
    academicYearSetup: "إعداد العام الدراسي",
    extraFees: "رسوم إضافية",
    receipts: "الإيصالات",
    monthlyPayroll: "كشف الرواتب الشهري",
    employees: "الموظفون",
    salaryHistory: "سجل الرواتب",
    expenseList: "قائمة المصروفات",
    additionalIncome: "دخل إضافي",
    categories: "الفئات",
    createExam: "إنشاء امتحان",
    enterMarks: "إدخال الدرجات",
    importMarks: "استيراد الدرجات",
    monitoring: "المتابعة",
    examGroups: "مجموعات الامتحانات",
    results: "النتائج",
    blockedStudents: "الطلاب الممنوعون",
    promoteStudents: "ترفيع الطلاب",
    graduatedStudents: "الطلاب الخريجون",
    history: "السجل",
    eligibilityRules: "شروط الترفيع",
    allQuizzes: "جميع الاختبارات",
    myQuizzes: "اختباراتي",
    createQuiz: "إنشاء اختبار",
    allUsers: "جميع المستخدمين",
    rolesPermissions: "الأدوار والصلاحيات",

    quickLinks: "روابط سريعة",
    comingSoon: "قريبًا",
    allSystemsOperational: "جميع الأنظمة تعمل",
    viewResults: "عرض النتائج",
    takeAttendance: "تسجيل الحضور",
  },

  dashboard: {
    totalStudents: "إجمالي الطلاب",
    totalTeachers: "إجمالي المعلمين",
    totalParents: "إجمالي أولياء الأمور",
    totalClasses: "إجمالي الصفوف",
    totalSubjects: "إجمالي المواد",
    feesOutstanding: "الرسوم المستحقة",
    totalCollected: "إجمالي المحصّل",
    income: "الإيرادات",
    expenses: "المصروفات",
    netIncome: "صافي الدخل",
    studentAttendance: "حضور الطلاب",
    present: "حاضر",
    absent: "غائب",
    late: "متأخر",
    registered: "مسجّل",
    quickActions: "إجراءات سريعة",
    addStudent: "إضافة طالب",
    addExpense: "إضافة مصروف",
    collectFees: "تحصيل الرسوم",
    takeAttendance: "تسجيل الحضور",
    createExam: "إنشاء امتحان",
    createQuiz: "إنشاء اختبار",
    sendNotice: "إرسال إشعار",
    generateReport: "إنشاء تقرير",
    recentActivities: "آخر الأنشطة",
    recentPayments: "آخر المدفوعات",
    upcomingExams: "الامتحانات القادمة",
    alertsNotifications: "التنبيهات والإشعارات",
    studentAdmissionTrend: "اتجاه تسجيل الطلاب",
    systemInformation: "معلومات النظام",
    serverStatus: "حالة الخادم",
    databaseStatus: "حالة قاعدة البيانات",
    online: "متصل",
    connected: "متصل",
    allSystemsNormal: "جميع الأنظمة تعمل بشكل طبيعي",
    viewAll: "عرض الكل",
    loadFailed: "تعذّر تحميل بيانات لوحة التحكم",
    todaysAttendance: "حضور اليوم",
    feeCollectionMonth: "تحصيل الرسوم (الشهر)",
    academicYear: "العام الدراسي",
    thisMonth: "+{count} هذا الشهر",
  },

  students: {
    loadingStudents: "جارٍ تحميل الطلاب…",
    students: "الطلاب",
    manageStudentRecordsRegistrationAndProfiles:
      "إدارة سجلات الطلاب والتسجيل والملفات الشخصية.",
    print: "طباعة",
    export: "تصدير",
    import: "استيراد",
    addStudent: "إضافة طالب",
    searchByIdNameParentOr: "ابحث بالمعرّف أو الاسم أو ولي الأمر أو الهاتف…",
    allClasses: "جميع الصفوف",
    allSections: "جميع الشعب",
    section: "الشعبة",
    allGenders: "كلا الجنسين",
    male: "ذكر",
    female: "أنثى",
    allStatus: "جميع الحالات",
    active: "نشط",
    inactive: "غير نشط",
    graduated: "متخرّج",
    clear: "مسح",
    student: "طالب",
    selected: "محدّد",
    deleteSelected: "حذف المحدّد",
    selectAllOnThisPage: "تحديد الكل في هذه الصفحة",
    studentId: "معرّف الطالب",
    name: "الاسم",
    gender: "الجنس",
    parent: "ولي الأمر",
    parentPhone: "هاتف ولي الأمر",
    class: "الصف",
    monthlyFee: "الرسوم الشهرية",
    regDate: "تاريخ التسجيل",
    status: "الحالة",
    actions: "إجراءات",
    viewProfile: "عرض الملف",
    edit: "تعديل",
    printProfile: "طباعة الملف",
    downloadProfile: "تنزيل الملف",
    delete: "حذف",
    resetDemoData: "إعادة تعيين بيانات العرض",
    deleteStudent: "حذف الطالب",
    deleteSelectedStudents: "حذف الطلاب المحددين",
  },

  topbar: {
    openMenu: "فتح القائمة",
    searchPlaceholder: "ابحث عن طالب أو معلم أو ولي أمر…",
    academicYear: "العام الدراسي:",
    notifications: "الإشعارات",
    profile: "الملف الشخصي",
    logout: "تسجيل الخروج",
    language: "اللغة",
    theme: "المظهر",
  },

  auth: {
    signIn: "تسجيل الدخول",
    signingIn: "جارٍ تسجيل الدخول…",
    username: "اسم المستخدم",
    password: "كلمة المرور",
    rememberMe: "تذكرني",
    invalidCredentials: "بيانات الدخول غير صحيحة",
    welcomeBack: "مرحبًا بعودتك",
    signInToContinue: "سجّل الدخول للمتابعة",
    notStaffQuestion: "لست موظفًا؟ سجّل الدخول من هنا:",
    parentPortalLink: "بوابة ولي الأمر",
    studentPortalLink: "بوابة الطالب",
    teacherPortalLink: "بوابة المعلم",
    checkResultsLink: "عرض النتائج",
    changePassword: "تغيير كلمة المرور",
    currentPassword: "كلمة المرور الحالية",
    newPassword: "كلمة المرور الجديدة",
    confirmPassword: "تأكيد كلمة المرور",
    passwordsDoNotMatch: "كلمتا المرور غير متطابقتين",
    passwordChanged: "تم تغيير كلمة المرور",
    idOrUsername: "المعرّف / اسم المستخدم",
    showPassword: "إظهار كلمة المرور",
    hidePassword: "إخفاء كلمة المرور",
    wrongCredentials:
      "اسم المستخدم أو كلمة المرور غير صحيحة. يرجى التحقق منهما والمحاولة مرة أخرى.",
    noSchoolLinked:
      "صفحة الدخول هذه غير مرتبطة بمدرسة. افتح عنوان مدرستك (مثل yourschool.ekulmis.com) وسجّل الدخول من هناك.",
    loginFailed: "فشل تسجيل الدخول. يرجى المحاولة مرة أخرى.",
  },
};

/**
 * The hand-written entries win: they were written for their screen, while the
 * generated ones come from matching English text across the whole system.
 */
export const ar = mergeDictionaries(arGenerated, curated);
