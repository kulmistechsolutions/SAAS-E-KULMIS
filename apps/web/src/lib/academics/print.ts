...
export function printTable(opts: PrintTableOptions) {
  ...
  <style>
    * { font-family: system-ui, -apple-system, Segoe UI, sans-serif; box-sizing: border-box; }
    ...
    ${printHeaderHtml(opts.academicYear ? `Academic Year: ${opts.academicYear}` : undefined)}
    <h2>${opts.title}</h2>
...
export function printClassReport(selectedKeys: string[], data: ClassReportData) {
  ...
  <style>
    * { font-family: system-ui, -apple-system, Segoe UI, sans-serif; box-sizing: border-box; }
    ...
    h2 { font-size:15px; margin:22px 0 8px; color:${school.primaryColor || "#4f46e5"}; }
    ...
    ${printHeaderHtml(`${escapeHtml(data.className)} · Academic Year ${escapeHtml(data.academicYear)} — Class Report`)}
    ${parts.join("")}
    <div class="foot">
