...
export default function PlatformSchoolDetailPage({
  ...
  async function toggleStatus() {
    ...
    if (next === "SUSPENDED" && !confirm(`Suspend "${school.name}"? Users will lose access.`)) return;
    try {
      const updated = await updateSchool(school.id, { status: next });
      setSchool(updated);
      toast(next === "SUSPENDED" ? "School suspended" : "School activated", "success");
    } catch (e) {
  ...
  async function handleDelete() {
    if (!school) return;
    if (!confirm(`Permanently delete "${school.name}" and ALL tenant data? This cannot be undone.`)) return;
    try {
  ...
  if (!school) {
    return (
      <div className="space-y-4">
        <Link href="/platform/schools" className="inline-flex items-center gap-2 text-sm text-violet-400">
  ...
  return (
    <div className="space-y-6">
      <Link href="/platform/schools" className="inline-flex items-center gap-2 text-sm text-violet-400 hover:underline">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 font-mono text-sm text-slate-400">{school.subdomain}</p>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-white/10 bg-white/5 p-5">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <dd>
                <a href={tenantUrl(school.subdomain)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-violet-400 hover:underline">
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 p-5">
          ...
          <div className="mt-4 space-y-4">
            <div>
              <label className="mb-1 block text-sm text-slate-400">School Name</label>
              <Input
                value={name}
