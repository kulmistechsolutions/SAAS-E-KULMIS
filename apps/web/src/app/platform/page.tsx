    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-white/10 bg-white/5 p-5">
        <div className="mb-4 flex items-center justify-between">
          ...
          <Link href="/platform/schools" className="text-sm text-violet-400 hover:underline">
            View all →
          </Link>
        </div>
            ...
            <tbody>
              {recent.map((s) => (
                <tr key={s.id} className="border-b border-white/5">
                  ...
                  <td className="px-3 py-3 font-mono text-slate-300">{s.subdomain}</td>
                  <td className="px-3 py-3"><SchoolStatusBadge status={s.status} /></td>
                  <td className="px-3 py-3 text-slate-300">{s.userCount}</td>
                  <td className="px-3 py-3 text-slate-400">{shortDate(s.createdAt)}</td>
                </tr>
              ))}
              {recent.length === 0 && (
                <tr>
